...
class Retail extends \Opencart\System\Engine\Controller {
   ...
   public function orders(): void {
    ...
    $results = $this->model_orders_retail->getDailyOrderSummary($filter);

    // ✅ ALSO PASS FILTER TO TOTAL (important)
    $total = $this->model_orders_retail->getTotalOrderDays($filter);
   ...
   public function salesByProduct(): void {
    ...
    $results = $this->model_orders_retail->getDailyProductReport($filter);

    // ✅ ALSO PASS FILTER TO TOTAL
    $total = $this->model_orders_retail->getTotalDays($filter);
   ...
   public function salesByCoupon(): void {
    ...
    // ✅ FILTERS (same as admin + API)
    $filter = [
        'filter_date_from' => $this->request->get['from_date'] ?? '',
        'filter_date_to'   => $this->request->get['to_date'] ?? '',
        'filter_phone'     => $this->request->get['phone'] ?? '',
    ...
    $total   = $this->model_orders_retail->getTotalCoupons($filter);
