<?php
class controllerapiauthenticator extends controller
{
    protected $length = 6;
    public function generateRandomSecret($secretLength = 16)
    {
		...
		$this->response->setOutput(json_encode(array("seckey"=>$secret)));
    }
    ...
    public function getQR()
    {
        ...
        $validate=$this->validate_getQR($this->request->post);
        if($validate['success']==1)
        {
            $name=$this->request->post['name'];
            $secret=$this->request->post['seckey'];
            $title = null;
            $params = array();
            ...
            $json=array(
                            "qrcode"=>'https://chart.googleapis.com/chart?chs='.$width.'x'.$height.'&chld='.$level.'|0&cht=qr&chl='.$urlencoded.'',
                            "seckey"=>$secret
                        );
        }else
            {
    ...
    protected function validate_getQR($raw)
    {
        $keys=array('name','seckey');
        foreach($keys as $key)
        {
    ...
    public function verifyCode($raw)
    {
        $code=$raw['totp'];
        $secret=$raw['seckey'];
        $discrepancy = 1;
    ...
    public function verifyTOTP()
    {
        $input = json_decode(file_get_contents("php://input"),true);
        $this->request->post=$input;
        $validate=$this->validate_verifyTOTP($this->request->post);
        if($validate['success']==1)
        {
            $json=$this->verifyCode($this->request->post);
        }else
            {
                $json=$validate;
            }
    ...
    protected function validate_verifyTOTP($raw)
    {
        $keys=array('totp','seckey');
        foreach($keys as $key)
        {
           if(!isset($raw[$key]) || empty($raw[$key]) || $raw[$key]=='')
            {
