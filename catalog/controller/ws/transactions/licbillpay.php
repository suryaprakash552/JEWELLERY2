<?php
class ControllerTransactionsLICBillpay extends Controller {
                ...
                if($pkg_info['exstatus'])
                {
                    ...
                    if($operator_info['exstatus'])
                    {
                        ...
                        if($service_assignment['exstatus'])
                        {
                            ...
                            if($api_margins_info['exstatus'])
                            {
                                $api_info=$this->model_transactions_common->getAPIInfo($api_margins_info['apiid'],$this->language->get('LICBILLPAYMENT'));
                                //print_r($api_info);
                                            ...
                                            if($api_margins_info['issurcharge']=="0")
                                            {
                                                ...
                                                if($wallet_debit)
                                                {
                                                    ...
                                                    $input=array(
                                                                     ...
                                                                     "Recharge_mode"=>$operator_info['servicename']."_LICPAY"
                                                                );
    ...
    public function execuiteCurlAPI($api,$raw,$addi)
    {
        if($api['method']==$this->language->get('POST'))
        {
            ...
            foreach($request as $key=>$value)
            {
                ...
                {
                    $o_request[$value]=json_decode($this->model_transactions_common->getOrderByClient($raw['fetchid'])['response'],true)['bill_fetch'];
                }
            }
            ...
            if(!empty($error) || $error)
            {
                   ...
                   $data['output']["message"] = "Your service provider may take upto 2 working days to consider the bill paid in their accounts.";
                   ...
                   return $data;
                    ...
                    if($response['status'] == $this->language->get('success'))
                    {
                       ...
                       $data['output']["ourrequestid"]=$addi['ourrequestid'];
                       $data['output']["fetchid"]  =  $raw['fetchid'];
                       $data["reference_id"]  =  $response['response']['refno'];
                       $data["apirequestid"]=$response['txnid'];
                       $data["apiid"]=$api['apiid'];
                       $data["url"]=$api['url'];
                       $data['output']['customername']=$raw['name'];
                       $data["request"]=$o_request;
                       ...
                       return $data;
                   }elseif($response['status'] == $this->language->get('failed'))
                   {
