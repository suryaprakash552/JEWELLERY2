<?php
class ControllerTransactionsFinoBenew extends Controller {
       ...
       // print_r($this->request->post);
        ...
        if($cust_info['exstatus'])
        {
            ...
            if($service_assignment['exstatus'])
            {
               ...
               if($enroll_info['exstatus'])
               {    
                 $api_info=$this->model_transactions_common->getAPIInfoByType($this->language->get('FINO_BE'));
                 ...
                 if($api_info['exstatus'])
                 {
                      ...
                      if($wallet_info['exstatus'])
                      {
                        $fino_info=$this->model_transactions_common->getEnrollmentByPhAEPSId($data['userid'],$enroll_info);  
                      //print_r($fino_info); 
                                ...
                                if($apiResponse['status']==$this->language->get('SUCCESS'))
                                {
                                    ...
                                    if($api_margins_info['issurcharge']==1)
                                    {
                                            $debit=array(
                                                                ...
                                                                "order_id"=>"0",
                                                                "description"=>$this->request->post['aepsid'].'#BE/AEPS-2#0#'.$this->request->post['uid'],
                                                                "transactiontype"=>'BE/AEPS-2',
                                      ...
                                      $result=json_decode($fcmResponse,true);
                                             if ($result){
                                                           $json['success']="1";
                                                            ...
                                                            $json['servicetype']='Bal Enq/AEPS-2';
