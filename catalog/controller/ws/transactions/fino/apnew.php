<?php
class ControllerTransactionsFinoApnew extends Controller {
    public function index($data)
    {
        ...
        if($cust_info['exstatus'])
        {
            ...
            if($service_assignment['exstatus'])
            {
               ...
               if($enroll_info['exstatus'])
               {    
                 $api_info=$this->model_transactions_common->getAPIInfoByType($this->language->get('FINO_AP'));
                               ...
                               $this->request->post['ourrequestid'] = $clientid;
                                ...
                                if($apiResponse['status'] == $this->language->get('SUCCESS'))
                                {
                                    $credit=array(
                                                        ...
                                                        "amount"=>$this->request->post['amount'],
                                                        "order_id"=>"0",
                                                        "description"=>$this->request->post['aepsid'].'#AP/AEPS-2#'.$this->request->post['amount'].'#'.$this->request->post['uid'],
                                                        "transactiontype"=>'AP/AEPS-2',
                                    ...
                                    $fcmResponse=$this->model_transactions_common->fcm_codeapi($record,$fcm_info);
                                      $result=json_decode($fcmResponse,true);
                                             if ($result){
                                                    ...
                                                    $json['servicetype']='AadharPay/AEPS-2';
                                                    $json['device'] = $this->request->post['device'];
                                                    $json['aepsid'] = $fino_info['aepsid'];
