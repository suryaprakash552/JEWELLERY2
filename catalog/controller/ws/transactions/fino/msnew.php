<?php
class ControllerTransactionsFinoMsnew extends Controller {
        ...
        if($cust_info['exstatus'])
        {
            ...
            if($service_assignment['exstatus'])
            {
               $pkg_info=$this->model_transactions_common->getPkgInfo($cust_info['packagetype']);
                ...
                if($pkg_info['exstatus'])
                {
               $enroll_info=$this->model_transactions_common->getaepscustid_new($cust_info['customer_id'], $this->request->post['telephone']);
                 //print_r($enroll_info);
               ...
               if($enroll_info['exstatus'])
               {    
                 $api_info=$this->model_transactions_common->getAPIInfoByType($this->language->get('FINO_MS'));
                 ...
                 if($api_info['exstatus'])
                 {
                      ...
                      if($wallet_info['exstatus'])
                      {
                        $fino_info=$this->model_transactions_common->getEnrollmentByPhAEPSIdfpfino($data['userid'],$enroll_info['aepsid']);  
                      //print_r($fino_info); 
                        $api_margins_info=$this->model_transactions_common->getMSMarginInfo_1($pkg_info['packageid'],10);
                        ...
                        if($wallet_debit)
                        {
                            ...
                            if($save_record['exstatus'])
                            {
                               ...
                               $this->request->post['aepsid'] = $fino_info['aepsid'];
                               $this->request->post['trunck'] = "bank2";
                               $this->request->post['ourrequestid'] = $clientid;
                              
                              $apiResponse=$this->callAPIfino($this->request->post,$api_info,$cust_info);
                                ...
                                if($apiResponse['status']==$this->language->get('SUCCESS'))
                                {
                                    ...
                                    if($api_margins_info['issurcharge']==1)
                                    {
                                            $debit=array(
                                                                ...
                                                                "order_id"=>"0",
                                                                "description"=>$this->request->post['aepsid'].'#MS/AEPS2#0#'.$this->request->post['uid'],
                                                                "transactiontype"=>'MS/AEPS2',
                                        ...
                                        do {
                                                if($parent_info['customer_group_id']=="2")
                                                {
                                                    $credit=array(
                                                                    ...
                                                                    "amount"=>$margins['dt'],
                                                                    "order_id"=>"0",
                                                                    "description"=>$this->request->post['aepsid'].'#MS/AEPS-2#0#'.$this->request->post['uid'],
                                                                    "transactiontype"=>'MS/AEPS-2',
                                                                    "transactionsubtype"=>$this->language->get('CREDIT'),
                                                                    "trns_type"=>$this->language->get('COMMISSION'),
                                                                    "txtid"=>$clientid
                                    ...
                                    $fcmResponse=$this->model_transactions_common->fcm_codeapi($record,$fcm_info);
                                      $result=json_decode($fcmResponse,true);
                                             if ($result){
                                                ...
                                                $json['servicetype']='MStatment/AEPS-2';
                                                ...
                                                $json['statement']=$apiResponse['statement'];
                                                $json['result'] = $result;
                                             }
        ...
        if(!empty($error) || $error)
        {
        ...
        }else
            {
                ...
                if(isset($response['response_code']) && $response['response_code']==1)
                {
                    ...
                    $request=$raw;
                    $response=$response;
                    foreach($response['ministatement'] as $statement)
                    {
                        $data[]=array(
                                        'date'=>$statement['date'],
                                        'txntype'=>$statement['txnType'],
                                        'amount'=>$statement['amount'],
                                        'narration'=>$statement['narration']
                                     );
                    }
                }else if(isset($response['response_code']) && in_array($response['response_code'],array(0,4,3,5,6,7,14,16,17,18,19,20,21,22,23,24,26,27,24,25,12,13,15,20,9,10,8,11)))
                {
                    $status='FAILURE';
