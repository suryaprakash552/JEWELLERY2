<?php
class ControllerTransactionsPaytmupiverify extends Controller {
    public function index($data)
    {
        ...
        if($cust_info['exstatus'])
        {
            $serviceInfo=$this->model_transactions_common->getServiceIdByName('UPI');
            ...
            if($service_assignment['exstatus'])
            {
                 ...
                 if($api_info['exstatus'])
                 {
                      ...
                      if($wallet_info['exstatus'])
                      {
                          if($wallet_info['amount']>1 && $wallet_info['amount']>=$this->config->get('config_dmt_upi_verify_price'))
                            {
                                $debit=array(
                                                "customerid"=>$cust_info['customer_id'],
                                                "amount"=>$this->config->get('config_dmt_upi_verify_price'),
                                                "order_id"=>"0",
                                                "description"=>'UPI_VERIFY#'.$this->request->post['beneficiaryVPA'].'#'.$this->config->get('config_dmt_upi_verify_price'),
                                                "transactiontype"=>'UPI_VERIFY',
                            ...
                            if($wallet_debit)
                            {
                                ...
                                $record=array(
                                                ...
                                                'admin'=>$this->config->get('config_dmt_upi_admin_verify_price'),
     ...
     public function apiCall($raw,$addi,$api_info)
     {
        ...
        */
        ...
        $paytmParams = array(
                                ...
                                "beneficiaryVPA" => $raw['beneficiaryVPA']
                            );
        //print_r($paytmParams);
