...
namespace Opencart\Catalog\Controller\Ws\Transactions\Paytm;
    class Accounttransfer extends \Opencart\System\Engine\Controller{
    public function index($data)
    {
        ...
        if($cust_info['exstatus'])
        {
           ...
           // print_r($serviceInfo);
            ...
            if($service_assignment['exstatus'])
            {
               $validate_sender=$this->model_ws_transactions_common->getSenderById($this->request->post);
               ...
               if($validate_sender['exstatus'])
               {
                   if($validate_sender['status'])
                   {
                       $validate_beneficiary=$this->model_ws_transactions_common->getBeneficiaryBySenderById($this->request->post,$validate_sender);
                       //print_r($validate_beneficiary);
                       if(!$validate_beneficiary['exstatus'])
                       {
                          $json['success']="0";
                          $json['message']=$this->language->get('error_beneficiary_notavailable'); 
                       }
                       
                       if($validate_beneficiary['exstatus'])
                       {
                          if(isset($validate_beneficiary['beneficiary']['status']))
                          {
                ...
                if($pkg_info['exstatus'])
                {
                             ...
                             if($api_info['exstatus'])
                             {
                                  ...
                                  if($wallet_info['exstatus'])
                                  {
                                      $api_margins_info=$this->model_ws_transactions_common->getDMTMarginInfo($pkg_info['packageid'],$this->request->post['amount']);
                                        ...
                                        if($api_margins_info['exstatus'])
                                        {
                                            ...
                                            if($api_margins_info['issurcharge']==0)
                                            {
                                                    if($wallet_info['amount']>1 && $wallet_info['amount']>=$this->request->post['amount'])
                                                    {
                                                        $debit=array(
                                                                        ...
                                                                        "order_id"=>"0",
                                                                        "description"=>'ACCOUNT_TRANSFER#'.$validate_beneficiary['beneficiary']['accountnumber'].'#'.$this->request->post['amount'].'#'.$validate_beneficiary['beneficiary']['ifsc'],
                                            ...
                                            if($wallet_debit)
                                            {
                                                ...
                                                $record=array(
                                                                ...
                                                                'remitterid'=>$this->request->post['remitterid'],
                                                                'snumber'=>$this->request->post['snumber'],
                                                                'ourrequestid'=>$clientid,
                                                                'yourrequestid'=>$this->request->post['yourrequestid'],
                                                                'accountnumber'=>$validate_beneficiary['beneficiary']['accountnumber'],
                                                                'ifsc'=>$validate_beneficiary['beneficiary']['ifsc'],
                                                                'bank'=>$validate_beneficiary['beneficiary']['bank'],
                                                                'amount'=>$this->request->post['amount'],
                                                                'profit'=>$margin_info['profit'],
                                                                'name'=>$validate_beneficiary['beneficiary']['name'],
                                                                'dt'=>$margin_info['dt'],
                                                                'sd'=>$margin_info['sd'],
                                                                'wt'=>$margin_info['wt'],
                                                ...
                                                if($save_record['exstatus'])
                                                {
                                                    ...
                                                    $beneficiaryName=$validate_beneficiary['beneficiary']['name'];
                                                        ...
                                                        if($api_margins_info['issurcharge']==0)
                                                            {
                                                                $credit=array(
                                                                                ...
                                                                                "auto_credit"=>"0",
                                                                                "order_id"=>"0",
                                                                                "description"=>'ACCOUNT_TRANSFER#'.$validate_beneficiary['accountnumber'].'#'.$this->request->post['amount'].'#'.$validate_beneficiary['ifsc'],
                                                                                "transactiontype"=>'ACCOUNT_TRANSFER',
                                                                                "transactionsubtype"=>$this->language->get('CREDIT'),
                                ...
                                {
                                    $json['success']="2";
                                    $json['message']=$this->language->get('error_beneficiary_suspend');
                                }
                       }
                   }else
                       {
       ...
       // $paytmParams["transferMode"]               = $raw['transferMode'];
        /*if($raw['transferMode']=="IMPS")
        {
        ...
        }else
            {
               ...
               $paytmParams["callbackUrl"]="http://nowpay.in/api/index.php?route=ws/bank.webhookcallbacks";
            }*/
    ...
    public function apiCallINSPaySync($raw,$addi,$api_info,$beneficiary,$cust_info)
     {
        ...
        $paytmParams=[
                        "payer" =>[
                                    ...
                                    "accountNumber" =>isset($param['userid_value'])
                                ],
                                "payee" => [
                                            "name" => $beneficiary['beneficiary']['name'],
                                            "accountNumber" => $beneficiary['beneficiary']['accountnumber'],
                                            "bankIfsc" => $beneficiary['beneficiary']['ifsc']
                                ],
        ...
        $token_value=isset($param['token_value']);
        $seckey_value=isset($param['seckey_value']);
        $curl = curl_init();
