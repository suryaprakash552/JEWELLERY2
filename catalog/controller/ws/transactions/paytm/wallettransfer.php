...
namespace Opencart\Catalog\Controller\Ws\Transactions\Paytm;
    class Wallettransfer extends \Opencart\System\Engine\Controller {
    public function index($data)
    {
        ...
        if($cust_info['exstatus'])
        {
            $serviceInfo=$this->model_ws_transactions_common->getServiceIdByName('PTMWALLET');
            ...
            if($service_assignment['exstatus'])
            {
                ...
                if($pkg_info['exstatus'])
                {
                 $api_info=$this->model_ws_transactions_common->getAPIInfoByType($this->language->get('TRANSFER_WALLET'));
                 ...
                 if($api_info['exstatus'])
                 {
                      ...
                      if($wallet_info['exstatus'])
                      {
                          $api_margins_info=$this->model_ws_transactions_common->getWALLETMarginInfo($pkg_info['packageid'],$this->request->post['amount']);
                            ...
                            if($api_margins_info['exstatus'])
                            {
                                ...
                                if($api_margins_info['issurcharge']=="0")
                                {
                                        if($wallet_info['amount']>1 && $wallet_info['amount']>=$this->request->post['amount'])
                                        {
                                            $debit=array(
                                                            ...
                                                            "order_id"=>"0",
                                                            "description"=>'WALLET_TRANSFER#'.$this->request->post['beneficiaryPhoneNo'].'#'.$this->request->post['amount'].'#'.$this->request->post['beneficiaryPhoneNo'],
                                ...
                                if($wallet_debit)
                                {
                                    ...
                                    $record=array(
                                                    ...
                                                    'name'=>$this->request->post['beneficiaryPhoneNo'],
                                                    'dt'=>$margin_info['dt'],
                                                    'sd'=>$margin_info['sd'],
                                                    'wt'=>$margin_info['wt'],
    ...
    public function apiCallINSPaySync($raw,$addi,$api_info,$cust_info)
     {
        ...
        $paytmParams=[
                        "payer" =>[
                                ...
                                "payee" => [
                                            "name" => $raw['beneficiaryPhoneNo'],
                                            "accountNumber" => $raw['beneficiaryPhoneNo'],
                                            "bankIfsc" => ""
                                ],
     ...
     public function apiCall($raw,$addi,$api_info)
     {
        ...
        */
        ...
        $paytmParams["beneficiaryPhoneNo"] = $raw['beneficiaryPhoneNo'];
        $paytmParams["amount"]             = $raw['amount'];
        $paytmParams["validateBeneficiary"]               = false;
