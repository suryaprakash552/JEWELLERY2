<?php
class ControllerTransactionsPaytmPayout extends Controller {
    public function index($data)
    {
        ...
        if($cust_info['exstatus'])
        {
            $serviceInfo=$this->model_transactions_common->getServiceIdByName('PAYOUT');
            ...
            if($service_assignment['exstatus'])
            {
                ...
                if($pkg_info['exstatus'])
                {
                 ...
                 if($api_info['exstatus'])
                 {
                      ...
                      if($wallet_info['exstatus'])
                      {
                          $api_margins_info=$this->model_transactions_common->getPAYOUTMarginInfo($pkg_info['packageid'],$this->request->post['amount']);
                                ...
                                if($api_margins_info['issurcharge']=="0")
                                {
                                        if($wallet_info['aeps_amount']>0 && $wallet_info['aeps_amount']>=$this->request->post['amount'])
                                        {
                                            $debit=array(
                                                            ...
                                                            "order_id"=>"0",
                                                            "description"=>'PAYOUT#'.$this->request->post['accountnumber'].'#'.$this->request->post['amount'].'#'.$this->request->post['ifsc'],
  ...
  /*      $curl = curl_init();
        ...
        //print_r($response);
        curl_close($curl);*/
        //381f5f20-916d-11ed-9110-3150f592e864:0f1960f16bf28331551776c0a52dc6d05bd91d3a
   $token_value="381f5f20-916d-11ed-9110-3150f592e864";
   $seckey_value="0f1960f16bf28331551776c0a52dc6d05bd91d3a";
        $curl = curl_init();
