...
namespace Opencart\Catalog\Controller\Ws\Transactions\Paytm;
    class Upitransfer extends \Opencart\System\Engine\Controller {
    public function index($data)
    {
        ...
        if($cust_info['exstatus'])
        {
            $serviceInfo=$this->model_ws_transactions_common->getServiceIdByName('UPI');
           // print_r($serviceInfo);
            ...
            if($service_assignment['exstatus'])
            {
                ...
                if($pkg_info['exstatus'])
                {
                 $api_info=$this->model_ws_transactions_common->getAPIInfoByType($this->language->get('TRANSFER_UPI'));
                 ...
                 if($api_info['exstatus'])
                 {
                      ...
                      if($wallet_info['exstatus'])
                      {
                          $api_margins_info=$this->model_ws_transactions_common->getUPIMarginInfo($pkg_info['packageid'],$this->request->post['amount']);
                                ...
                                if($api_margins_info['issurcharge']=="0")
                                {
                                        if($wallet_info['amount']>1 && $wallet_info['amount']>=$this->request->post['amount'])
                                        {
                                            $debit=array(
                                                            ...
                                                            "order_id"=>"0",
                                                            "description"=>'UPI_TRANSFER#'.$this->request->post['beneficiaryVPA'].'#'.$this->request->post['amount'].'#'.$this->request->post['beneficiaryVPA'],
