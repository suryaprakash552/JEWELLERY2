<?php
class ControllerTransactionsRoffers extends Controller {
    public function index($data)
    {
        ...
        if($cust_info['exstatus'])
        {
            ...
            if($operator_info['exstatus'])
            {
                $api_info=$this->model_transactions_common->getAPIInfoByType($this->language->get('GET_ROFFER'));
        ...
        if($api['method']==$this->language->get('GET'))
        {
        	 ...
        	 if($api['testmode']==$this->language->get('LIVE'))
              ...
              }elseif($api['testmode']==$this->language->get('TEST_FAILED'))
              {
               ...
               if($response[$resparams['status']]==$resparams['success_status_value'] && isset($response['records']))
                {
                    for($i=0;$i<sizeof($response['records']);$i++)
                    {
                        $rofers[]=[
                                    "rs"=>isset($response['records'][$i]['rs'])?$response['records'][$i]['rs']:'0',
                                    "desc"=>isset($response['records'][$i]['desc'])?$response['records'][$i]['desc']:'NA'
                                  ];
                    }
                    $res=[
                            "tel"=>$raw['number'],
