<?php
class ControllerTransactionsFpayaepsAp extends Controller {
    public function index($data)
    {
        ...
        if($cust_info['exstatus'])
        {
            ...
            if($service_assignment['exstatus'])
            {
               ...
               if($fpaeps_info['exstatus'])
               {    
                 $api_info=$this->model_transactions_common->getAPIInfoByType($this->language->get('FPAY_AP'));
                 ...
                 if($api_info['exstatus'])
                 {
                      ...
                      if($wallet_info['exstatus'])
                      {
                        $api_margins_info=$this->model_transactions_common->getAPMarginInfo_1($pkg_info['packageid'],$this->request->post['amount']);
                        ...
                        if($wallet_debit)
                        {
                            ...
                            if($save_record['exstatus'])
                            {
                                ...
                                $newdata =array(
                                     "pid" =>$this->request->post['PidData'],
                                     "adhaarNumber" =>$this->request->post['uid'],
                                     "nationalBankIdentificationNumber" =>$this->request->post['bankid'],
                                     "mobileNumber" => $this->request->post["mobilenumber"], 
                                     "transactionType" => "AP", 
                                     "merchantUserName" => $this->request->post["telephone"],
                                     "merchantPin" => $fpaeps_info['customerid'],
                                     "transactionAmount"=>$this->request->post["amount"],
                                     "device"=>$this->request->post['device']
                                     );   
                                  //print_r($newdata);   
                                $apiResponse=$this->callAPIFP($newdata,$additional,$api_info,$cust_info);
                                //print_r($apiResponse);
                                if($apiResponse['status']==$this->language->get('SUCCESS'))
                                {
                                    $credit=array(
                                                        ...
                                                        "description"=>$this->request->post['aepsid'].'#AP#'.$this->request->post['amount'].'#'.$this->request->post['uid'],
                                                        "transactiontype"=>'AP',
                                    ...
                                    $this->model_transactions_common->updateFPAEPSRecord($response);
    ...
    public function callAPIFP($raw,$addi,$api_info,$cust_info)
     {
         $raw['merchantTransactionId'] = $addi['ourrequestid'];
          $curl = curl_init();
