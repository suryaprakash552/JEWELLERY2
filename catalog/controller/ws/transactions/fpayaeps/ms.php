<?php
class ControllerTransactionsFpayaepsMs extends Controller {
     ...
     $json=array();
        ...
        if($cust_info['exstatus'])
        {
            ...
            if($service_assignment['exstatus'])
            {
               ...
               if($fpaeps_info['exstatus'])
               {    
                 $api_info=$this->model_transactions_common->getAPIInfoByType($this->language->get('FPAY_MS'));
                            ...
                            if($apiResponse['status']==$this->language->get('SUCCESS'))
                                {
                                    $credit=array(
                                                        ...
                                                        "description"=>$this->request->post['aepsid'].'#MS/AEPS-1#'.'#'.$this->request->post['uid'],
                                                        "transactiontype"=>'MS/AEPS-1',
                                    ...
                                    $fcmResponse=$this->model_transactions_common->fcm_codeapi($record,$fcm_info);
                                      $result=json_decode($fcmResponse,true);
                                             if ($result){
                                                    ...
                                                    $json['servicetype']='MStatment/AEPS-1';
               ...
               //print_r($response['data']);
                if(isset($response['data']['responseCode']) && $response['data']['responseCode']=='00')
                {
                    ...
                    $rrn=isset($response['data']['bankRRN'])?$response['data']['bankRRN']:'';
                     $request=$raw;
                    $response=$response;
                    foreach($response['data']['miniStatementStructureModel'] as $statement)
                    {
                        $data[]=array(
                                        'date'=>$statement['date'],
