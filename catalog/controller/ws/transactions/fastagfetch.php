<?php
class ControllerTransactionsFastagfetch extends Controller {
    public function index($data)
    {
        ...
        if($cust_info['exstatus'])
        {
            ...
            if($operator_info['exstatus'])
            {
                ...
                if($service_assignment['exstatus'])
                {
                    $api_billfetch_info=$this->model_transactions_common->getBillFetchAPIInfo($this->language->get('FASTAGFETCH'));
                    ...
                    if($api_billfetch_info['exstatus'])
                    {
                        ...
                        $input=array(
                                         ...
                                         "auto_status" => '0',
                                         "Recharge_mode"=>$operator_info['servicename']."_FETCH"
                                    );
            ...
            }else
                {
                    ...
                    if(isset($response['response']) || !empty($response['response']))
                    {
                        ...
                        if($response['status'])
                        {
                                   ...
                                   $data['output']["message"] = "Transaction Success";
                                   $data['output']["number"]  =  $raw['number'];
                                   $data['output']["amount"]  =  isset($response['amount'])?$response['amount']:'0';
                                   $data['output']["dueamount"]  =  isset($response['amount'])?$response['amount']:'0';
                                   $data['output']["duedate"]  =  isset($response['duedate'])?$response['duedate']:'NA';
                                   $data['output']["customername"]  =  !empty($response['name'])?$response['name']:"NA";
                                   $data['output']["mobile"]=$raw['mobile'];
                                   $data['output']["ourrequestid"]=$addi['ourrequestid'];
                                   $data['output']["fetchid"]  =  $addi['ourrequestid'];
                                   $data["reference_id"]  =  $addi['ourrequestid'];
                                   $data["apiid"]=$api['apiid'];
                                   $data['apirequestid']='';
                                   $data["bill_fetch"]  =  isset($response['bill_fetch'])?$response['bill_fetch']:'';
                                   $data["url"]=$api['url'];
                                   $data["request"]=$o_request;
                                   ...
                                   $data['output']["message"] = 'Transaction Failed';
                                   $data['output']["number"]  =  $raw['number'];
                                   $data['output']["amount"]  =  "0";
                                   $data['output']["dueamount"]  =  '0';
                                   $data['output']["duedate"]  =  'NA';
