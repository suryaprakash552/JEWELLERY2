<?php
class ControllerTransactionsheavyrefresh extends Controller {
    public function index($data)
    {
        ...
        if($cust_info['exstatus'])
        {
            ...
            if($operator_info['exstatus'])
            {
                $api_info=$this->model_transactions_common->getAPIInfoByType($this->language->get('HEAVYREFRESH'));
                ...
                if($api_info['exstatus'])
                {
                    if($data['source'] != "API"){ 
                           $input=array(
                                     ...
                                     "count" => 1,
                                     "status" => '2',
                                     "type" => $operator_info['servicename'],
                        ...
                        if($wallet_info['plan_limit']>0)
                        {
                            ...
                            $wallet_debit=$this->model_transactions_common->doPlanWalletDebit($debit);
                                ...
                                if($wallet_debit)
                                {
                                    ...
                                    if($create_record)
                                    {
                                        $additional=array("mappingcode"=>$operator_info['operater_code']);
        ...
        if($api['method']==$this->language->get('GET'))
        {
        	 ...
        	 if($api['testmode']==$this->language->get('LIVE'))
              ...
              }elseif($api['testmode']==$this->language->get('TEST_FAILED'))
              {
                ...
                {
                        $refresh=[
                                      "desc"=> $response['records']['desc'],
                                      "customerName"=> $response['records']['customerName'],
