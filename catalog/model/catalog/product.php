...
class Product extends \Opencart\System\Engine\Model {
	...
	public function getProduct(int $product_id): array {
		...
		if ($query->num_rows) {
			...
			$product_data['mrp'] = (float)($query->row['price']);
			
			$product_data['provided_discount'] = $query->row['discount']?((float)$query->row['price']-(float)$query->row['discount']):0;
