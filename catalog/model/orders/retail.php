...
class Retail extends \Opencart\System\Engine\Model {
    ...
    public function getTotalOrders(): int {
      ...
      public function getSellerSummary(array $data = []): array {
        ...
        FROM (
            ...
            WHERE o.customer_group_id = 2 AND o.sellerid IS NOT NULL
            GROUP BY o.order_id
        ) t
    ...
    public function getTotalSellerSummary(): int {
        return (int)$this->db->query(
            "SELECT COUNT(DISTINCT sellerid) AS total FROM `" . DB_PREFIX . "order`
             WHERE sellerid IS NOT NULL AND customer_group_id = 2"
        )->row['total'];
    }
    ...
    public function getTotalSalesByNumber(): int {
        return (int)$this->db->query("
            SELECT COUNT(*) AS total FROM (
                SELECT DATE(date_added), sellerid
                FROM `" . DB_PREFIX . "order`
                WHERE sellerid IS NOT NULL AND customer_group_id = 2
                GROUP BY DATE(date_added), sellerid
            ) x
        ")->row['total'];
    }
    ...
    public function getSalesByCoupon(array $data = []): array {
        $sql = "
            ...
            FROM (
                ...
                LEFT JOIN `" . DB_PREFIX . "product` p ON p.product_id = op.product_id
                LEFT JOIN `" . DB_PREFIX . "order_invoice` inv ON inv.order_id = o.order_id
                WHERE o.customer_group_id = 2 AND inv.coupon IS NOT NULL AND inv.coupon != ''
        ";
        ...
        $sql .= "
            ...
            LEFT JOIN `" . DB_PREFIX . "customer` c ON c.customer_id = t.sellerid
