...
class Reward extends \Opencart\System\Engine\Model {
	...
	public function updateReward(array $data): void
{
    $customer_id = (int)$data['customer_id'];
    $order_id    = (int)$data['order_id'];
    $points      = (float)$data['points'];
...
public function clearAllRewards(int $customer_id): void
{
    $this->db->query("
        UPDATE `" . DB_PREFIX . "customer_reward`
        SET status = 'clear',
            description = CONCAT(description, ' (CLEARED DUE TO REDEEM)')
