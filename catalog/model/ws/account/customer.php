...
class ModelAccountCustomer extends Model {
	public function addCustomer($data) {
        ...
        if (isset($data['customer_package_id'])) {
			$customer_package_id = $data['customer_package_id'];
		} else {
			$customer_package_id = $this->config->get('config_customer_package_id');
		}
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX . "manage_wallet` SET customerid = '" . $customer_id . "', amount=0");
		$this->db->query("INSERT INTO `" . DB_PREFIX . "customer_transaction` SET customer_id = '" . $customer_id . "', amount=0,balance=0,order_id=0,trns_type='FORWARD',txtid='".date('Ymdhis')."',description='REGISTRATION',transactiontype='REGISTRATION',transactionsubtype='DEBIT',date_added=now()");
