...
class Product extends \Opencart\System\Engine\Controller {
	...
	public function getList(): string {
		...
		foreach ($results as $result) {
            ...
            if ($product_id > 0) {
                ...
                foreach ($product_discounts as $product_discount) {
                    if (
                        ($product_discount['date_start'] == '0000-00-00' || strtotime($product_discount['date_start']) < time()) &&
                        ($product_discount['date_end'] == '0000-00-00' || strtotime($product_discount['date_end']) > time())
                    ) {
			...
			$data['products'][] = [
				...
				'special' => $special,
				'pos_quentity'=>$result['pos_quentity'],
		...
		if ($filter_rack_code !== '') {
            $url .= '&filter_rack_code=' . urlencode($filter_rack_code);
        }
        
        if ($filter_box_id !== '') {
            $url .= '&filter_box_id=' . urlencode($filter_box_id);
        }
        
        if ($filter_barcode !== '') {
            $url .= '&filter_barcode=' . urlencode($filter_barcode);
        }
        ...
        $data['sort_box_id'] = $this->url->link('catalog/product.list', 'user_token=' . $this->session->data['user_token'] . '&sort=p.box_id' . $url);
		$url = '';
	...
	public function form(): void {
        ...
        if (!empty($product_info)) {
            $data['short_code'] = $product_info['short_code'];
        } else {
            $data['short_code'] = '';
        }
		...
		// Received Price
        if (!empty($product_info)) {
            $data['received_price'] = $product_info['received_price'];
        } else {
            $data['received_price'] = '';
        }
	...
	if (!empty($product_info)) {
            ...
            $percentage  = (float)($product_info['additional_price'] ?? 0);
        
            if ($percentage > 0) {
                $base_price = $final_price / (1 + ($percentage / 100));
            } else {
                $base_price = $final_price;
            }

            $data['price'] = number_format($base_price, 2, '.', '');
		...
		// Additional Price %
        if (!empty($product_info)) {
            $data['additional_price'] = $product_info['additional_price'] ?? '';
        } else {
		...
		if (!empty($product_info)) {
			$data['max_quantity'] = $product_info['max_quantity'];
		} else {
			$data['max_quantity'] = '';
		}
		if (!empty($product_info)) {
			$data['r_tag'] = $product_info['r_tag'];
		} else {
			$data['r_tag'] = '';
		}
		if (!empty($product_info)) {
			$data['w_tag'] = $product_info['w_tag'];
		} else {
			$data['w_tag'] = '';
		}
			...
			if (isset($product_option['product_option_value'])) {
				foreach ($product_option['product_option_value'] as $product_option_value) {
					...
					if ($option_value_info) {
                        $points = isset($product_option_value['points']) && $product_option_value['points'] !== ''
                            ? round((float)$product_option_value['points'])
                            : 0;
                    
                        $weight = isset($product_option_value['weight']) && $product_option_value['weight'] !== ''
                            ? round((float)$product_option_value['weight'])
                            : 0;
	...
	public function save(): void {
		...
		if (!empty($post_info['upc'])) {
            $post_info['sku'] = '';
        } else {
            $post_info['upc'] = '';
        }
        $r_tag_input = $post_info['r_tag'] ?? 'R';
        $w_tag_input = $post_info['w_tag'] ?? 'W';
        
        $r_value = preg_replace('/[^0-9]/', '', $r_tag_input);
        $w_value = preg_replace('/[^0-9]/', '', $w_tag_input);

        $post_info['r_tag'] = 'R' . $r_value;
        $post_info['w_tag'] = 'W' . $w_value;
        ...
        $base_price = (float)$post_info['price'];
        $additional_percentage = isset($post_info['additional_price']) ? (float)$post_info['additional_price'] : 0;
        
        $final_price = $base_price + ($base_price * ($additional_percentage / 100));
        
        $post_info['price'] = $final_price;
        
        $post_info['additional_price'] = $additional_percentage;
        $this->load->model('catalog/product');
        
        // Unit fallback
        if (empty($this->request->post['sku']) && empty($this->request->post['upc'])) {
            $this->request->post['sku'] =
                $this->model_catalog_product->getNextUnitBarcode();
        }
        
        // Box fallback
        if (!empty($this->request->post['upc'])) {
            // already generated safely
        }
        
        if (!empty($post_info['box_id'])) {

            $box = $this->model_catalog_product->getBoxById($post_info['box_id']);
        ...
        if (!empty($post_info['master_id']) && empty($post_info['image'])) {
            $post_info['image'] = 'no_image.png';
        }
		...
		if (empty($post_info['sku']) && empty($post_info['upc'])) {
            $json['error']['sku'] = 'Please enter the Barcode ID!';
        }
        ...
        if (empty($post_info['short_code'])) {
            $this->load->model('catalog/product');
            $post_info['short_code'] = $this->model_catalog_product->generateShortCode();
        }
...
public function generateBarcode(): void {
    ...
    if ($type === 'box') {
        $barcode = $this->model_catalog_product->generateBoxBarcode();
        $field = 'upc';
    } else {
        $barcode = $this->model_catalog_product->getNextUnitBarcode();
        $field = 'sku';
    }
	...
	public function getBoxProductName(): void {

    $json = [];

    $box_id = (int)($this->request->get['box_id'] ?? 0);

    if (!$box_id) {
        $json['success'] = false;
        $json['message'] = 'Missing box_id';
    } else {

        // Load product model
        $this->load->model('catalog/product');

        $product = $this->model_catalog_product->getProduct($box_id);
