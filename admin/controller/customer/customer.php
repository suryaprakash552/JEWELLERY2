...
class Customer extends \Opencart\System\Engine\Controller {
	...
	public function form(): void {
		...
		if (isset($this->request->get['filter_gst'])) {
			$url .= '&filter_gst=' . urlencode(html_entity_decode($this->request->get['filter_gst'], ENT_QUOTES, 'UTF-8'));
		}
		...
		if (isset($this->request->get['customer_id'])) {
         ...
         $banklocal_info = $this->model_customer_customer->getLocal((int)$this->request->get['customer_id']);
         $banknational_info = $this->model_customer_customer->getNational((int)$this->request->get['customer_id']);
         $personal_info = $this->model_customer_customer->getPersonal((int)$this->request->get['customer_id']);
        $kyc_raw = $this->model_customer_customer->getKYCInfo($this->request->get['customer_id']);
        $kyc_info = [];
            foreach ($kyc_raw as $row) {
                $kyc_info[(int)$row['idtype']] = $row;
                }
		...
		if (!empty($banklocal_info)) {
        $data['cif']       = $banklocal_info['cif'];
        $data['userid']    = $banklocal_info['userid'];
        $data['accountno'] = $banklocal_info['accountno'];
        $data['actype']    = $banklocal_info['actype'];
        $data['branch']    = $banklocal_info['branch'];
        $data['Pennydropamount']    = $banklocal_info['Pennydropamount'];
        $data['Pennydroptax']    = $banklocal_info['Pennydroptax'];
            ...
            $data['actype'] = '';
            $data['branch'] = '';
            $data['Pennydropamount'] = '';
            $data['Pennydroptax'] = '';
            
        }

        if (!empty($banknational_info)) {
            $data['ifsc'] = $banknational_info['ifsc'];
            $data['acno'] = $banknational_info['acno'];
            $data['bankname'] = $banknational_info['bankname'];
            $data['acname']  = $banknational_info['acname'];
            $data['acctype']    = $banknational_info['acctype'];
            $data['branchname']    = $banknational_info['branchname'];
        } else {
            $data['ifsc'] = '';
            $data['acno'] = '';
            $data['bankname'] = '';
        ...
        $data['date_of_birth']       = $personal_info['date_of_birth'];
        $data['customer_gender']    = $personal_info['customer_gender'];
        $data['fathername'] = $personal_info['fathername'];
        $data['company_name']    = $personal_info['company_name'];
        $data['gst']    = $personal_info['gst'];
        $data['selected_package'] = $personal_info['packageid'];
		...
		/*if (!empty($kyc_info)) {
			$data['kycpanidno'] = $kyc_info['kycpanidno'];
		} else {
			$data['kycpanidno'] = '';
		}
		if (!empty($kyc_info)) {
			$data['kycaadharidno'] = $kyc_info['kycaadharidno'];
		} else {
			$data['kycaadharidno'] = '';
		}
		if (!empty($kyc_info)) {
			$data['kycprofileidno'] = $kyc_info['kycprofileidno'];
		} else {
			$data['kycprofileidno'] = '';
		}
		if (!empty($kyc_info)) {
			$data['kycshopidno'] = $kyc_info['kycshopidno'];
		} else {
			$data['kycshopidno'] = '';
		}
		if (!empty($kyc_info)) {
			$data['kycpanimage'] = $kyc_info['kycpanimage'];
		} else {
			$data['kycpanimage'] = 'no_image.png';
		}
		if (!empty($kyc_info)) {
			$data['kycaadharimage'] = $kyc_info['kycaadharimage'];
		} else {
			$data['kycaadharimage'] = 'no_image.png';
		}
		if (!empty($kyc_info)) {
			$data['kycprofileimage'] = $kyc_info['kycprofileimage'];
		} else {
			$data['kycprofileimage'] = 'no_image.png';
		}
		if (!empty($kyc_info)) {
			$data['kycshopimage'] = $kyc_info['kycshopimage'];
		} else {
			$data['kycshopimage'] = 'no_image.png';
		}*/


        $this->load->model('tool/image');
        
        $data['pan'] = !empty($kyc_info[0]['idno']) ? $kyc_info[0]['idno'] : '';
        $data['panplaceholder'] = $this->model_tool_image->resize('no_image.png',$this->config->get('config_image_default_width'),$this->config->get('config_image_default_height'));
        
        if (!empty($kyc_info[0]['image']) && is_file(DIR_IMAGE . $kyc_info[0]['image'])) {
            $data['panimageinput'] = $kyc_info[0]['image'];
            $data['panimage'] = $this->model_tool_image->resize($kyc_info[0]['image'],$this->config->get('config_image_default_width'),$this->config->get('config_image_default_height'));
        }
        else {
        ...
        $data['aadhar'] = !empty($kyc_info[1]['idno']) ? $kyc_info[1]['idno'] : '';
        $data['aadharplaceholder'] = $this->model_tool_image->resize('no_image.png',$this->config->get('config_image_default_width'),$this->config->get('config_image_default_height'));
        
        if (!empty($kyc_info[1]['image']) && is_file(DIR_IMAGE . $kyc_info[1]['image'])) {
            $data['aadharimageinput'] = $kyc_info[1]['image'];
            $data['aadharimage'] = $this->model_tool_image->resize($kyc_info[1]['image'],$this->config->get('config_image_default_width'),$this->config->get('config_image_default_height'));
        }
        else {
        ...
        $data['profile'] = !empty($kyc_info[3]['idno']) ? $kyc_info[3]['idno'] : '';
        $data['profileplaceholder'] = $this->model_tool_image->resize('no_image.png',$this->config->get('config_image_default_width'),$this->config->get('config_image_default_height'));
        
        if (!empty($kyc_info[3]['image']) && is_file(DIR_IMAGE . $kyc_info[3]['image'])) {
            $data['profileimageinput'] = $kyc_info[3]['image'];
            $data['profileimage'] = $this->model_tool_image->resize($kyc_info[3]['image'],$this->config->get('config_image_default_width'),$this->config->get('config_image_default_height'));
        } 
        else {
            $data['profileimageinput'] = 'no_image.png';
            $data['profileimage'] = $data['profileplaceholder'];
        }
        
        $data['shop'] = !empty($kyc_info[4]['idno']) ? $kyc_info[4]['idno'] : '';
        $data['shopplaceholder'] = $this->model_tool_image->resize('no_image.png',$this->config->get('config_image_default_width'),$this->config->get('config_image_default_height'));
        
        if (!empty($kyc_info[4]['image']) && is_file(DIR_IMAGE . $kyc_info[4]['image'])) {
            $data['shopimageinput'] = $kyc_info[4]['image'];
            $data['shopimage'] = $this->model_tool_image->resize($kyc_info[4]['image'],$this->config->get('config_image_default_width'),$this->config->get('config_image_default_height'));
        }
        else {
            $data['shopimageinput'] = 'no_image.png';
            $data['shopimage'] = $data['shopplaceholder'];
        }
        
    }
    $this->load->model('recharge/packages');
    $data['packages'] = $this->model_recharge_packages->getPackages();
    
    $personal_info = $this->model_customer_customer->getPersonal((int)$this->request->get['customer_id']);
    
    if (isset($this->request->post['packagename'])) {
        $data['selected_package'] = $this->request->post['packagename'];
    } elseif (!empty($personal_info)) {
        $data['selected_package'] = $personal_info['packageid']; // 🔹 use ID instead of name
    } else {
        $data['selected_package'] = '';
    }
    ...
    $data['amount'] = $this->currency->format(isset($transaction_total['amount'])?$transaction_total['amount']:0, $this->config->get('config_currency'));
    $data['aepsbalance'] = $this->currency->format(isset($transaction_total['aeps_amount'])?$transaction_total['aeps_amount']:0, $this->config->get('config_currency'));
	...
	public function save(): void {
	 ...
	 if ($post_info['authpin'] || !isset($post_info['customer_id'])) {
			$authpin = html_entity_decode($post_info['authpin'], ENT_QUOTES, 'UTF-8');
		
		if (!oc_validate_length($authpin, $this->config->get('config_auth_length'), 4)) {
				$json['error']['authpin'] = sprintf($this->language->get('error_password_length'), $this->config->get('config_auth_length'));
			}
			$required = [];
			
			if ($this->config->get('config_password_number') && !preg_match('/[0-9]/', $authpin)) {
				$required[] = $this->language->get('error_password_number');
			}
			if ($required) {
				$json['error']['authpin'] = sprintf($this->language->get('error_password'), implode(', ', $required), $this->config->get('config_auth_length'));
			}
		...
		if (!$json) {
            if (empty($post_info['customer_id'])) {
                $customer_id = $this->model_customer_customer->addCustomer($post_info);
	...
	public function getTransaction(): string {
		...
		//$data['amount'] = $this->currency->format($this->model_customer_customer->getTransactionTotal($customer_id)['amount'], $this->config->get('config_currency'));
	//	$data['aepsbalance'] = $this->currency->format($this->model_customer_customer->getTransactionTotal($customer_id)['aeps_amount'], $this->config->get('config_currency'));
	...
	public function addTransaction(): void {
		...
		if (!$json) {
			...
			$this->model_customer_customer->addTransaction($customer_id, (string)$post_info['description'].'# OfflineByAdmin', (float)$post_info['amount'],(string)$post_info['transactiontype'], (string)$post_info['transactionsubtype']);
