...
class Enrollments extends \Opencart\System\Engine\Controller {	private $error = array();

	public function index() {
		$this->load->language('matm/enrollments');

		$this->document->setTitle($this->language->get('heading_title'));

		$this->load->model('matm/enrollments');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_matm_enrollments->addProduct($this->request->post);
    		...
    		if (isset($this->request->get['filter_matmid'])) {
    			$url .= '&filter_matmid=' . urlencode(html_entity_decode($this->request->get['filter_matmid'], ENT_QUOTES, 'UTF-8'));
    		}
			...
			$this->response->redirect($this->url->link('matm/enrollments', 'user_token=' . $this->session->data['user_token'] . $url, true));
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_matm_enrollments->editProduct($this->request->get['id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $product_id) {
				$this->model_matm_enrollments->deleteProduct($product_id);
			}
	...
	protected function getList() 
	{
		...
		if (isset($this->request->get['filter_matmid'])) {
			$filter_matmid = $this->request->get['filter_matmid'];
		} else {
			$filter_matmid = '';
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('matm/enrollments', 'user_token=' . $this->session->data['user_token'] . $url, true)
		);

		$data['add'] = $this->url->link('matm/enrollments.add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['copy'] = $this->url->link('matm/enrollments.copy', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['delete'] = $this->url->link('matm/enrollments.delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['export'] = $this->url->link('matm/enrollments.export', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$product_total = $this->model_matm_enrollments->getTotalProducts($filter_data);

		$results = $this->model_matm_enrollments->getProducts($filter_data);
        $i=1;
		foreach ($results as $result) 
		{
			...
			$data['products'][] = array(
				...
				'edit'       => $this->url->link('matm/enrollments.edit', 'user_token=' . $this->session->data['user_token'] . '&id=' . $result['id'] . $url, true)
			);
			$i++;
		}
		...
		$data['sort_mobilenumber'] = $this->url->link('matm/enrollments', 'user_token=' . $this->session->data['user_token'] . '&sort=p.mobilenumber' . $url, true);
		$data['sort_matmid'] = $this->url->link('matm/enrollments', 'user_token=' . $this->session->data['user_token'] . '&sort=p.matmid' . $url, true);
		$data['sort_created'] = $this->url->link('matm/enrollments', 'user_token=' . $this->session->data['user_token'] . '&sort=p.created' . $url, true);
		$data['sort_status'] = $this->url->link('matm/enrollments', 'user_token=' . $this->session->data['user_token'] . '&sort=p.status' . $url, true);
		$data['sort_order'] = $this->url->link('matm/enrollments', 'user_token=' . $this->session->data['user_token'] . '&sort=p.sort_order' . $url, true);
		...
		$data['filter_matmid']=$filter_matmid;
		...
		$this->response->setOutput($this->load->view('matm/enrollments_list', $data));
	}
	...
	protected function getForm() {
		...
		if (isset($this->error['matmid'])) {
			$data['error_matmid'] = $this->error['matmid'];
		} else {
			$data['error_matmid'] = '';
		}
		...
		if (!isset($this->request->get['id'])) {
			$data['action'] = $this->url->link('matm/enrollments.add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('matm/enrollments.edit', 'user_token=' . $this->session->data['user_token'] . '&id=' . $this->request->get['id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('matm/enrollments', 'user_token=' . $this->session->data['user_token'] . $url, true);

		if (isset($this->request->get['id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$product_info = $this->model_matm_enrollments->getProduct($this->request->get['id']);
			$default_address_info = $this->model_matm_enrollments->getmatmAddress($this->request->get['id'],'0');
			$office_address_info = $this->model_matm_enrollments->getmatmAddress($this->request->get['id'],'1');
		}
		...
		}elseif(isset($product_info['mobilenumber']))
		{
		    ...
		    if(isset($this->request->post['matmid']))
		{
		    $data['matmid']=$this->request->post['matmid'];
		}elseif(isset($product_info['matmid']))
		{
		    $data['matmid']=$product_info['matmid'];
		}else
		    {
		        $data['matmid']='';
		    }
		...
		if (isset($this->request->post['product_image'])) {
			...
			$product_images = $this->model_matm_enrollments->getProductImages($this->request->get['id']);
		} else {
		...
		$this->response->setOutput($this->load->view('matm/enrollments_form', $data));
	}
    
	protected function validateForm() 
	{
		if (!$this->user->hasPermission('modify', 'matm/enrollments')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
		
		if(!isset($this->request->post['customerid']) || empty($this->request->post['customerid']))
		{
		    $this->error['customerid'] = $this->language->get('error_customerid');
		}
		
		if(isset($this->request->post['customerid']) && !empty($this->request->post['customerid']) && !isset($this->request->get['id']))
		{
		    $this->load->model('customer/customer');
		    $cust_info=$this->model_customer_customer->getCustomer($this->request->post['customerid']);
		    if(isset($cust_info['customer_id']) && !empty($cust_info['customer_id']))
		    {}else{
		        $this->error['no_customerid'] = $this->language->get('error_no_customerid');
		    }
		}
		...
		if(!isset($this->request->post['middlename']) || empty($this->request->post['middlename']))
		{
		    $this->error['middlename'] = $this->language->get('error_middlename');
		}
		
		if(!isset($this->request->post['mobilenumber']) || empty($this->request->post['mobilenumber']))
		{
		    $this->error['mobilenumber'] = $this->language->get('error_mobilenumber');
		}
		if(isset($this->request->post['mobilenumber']) && !empty($this->request->post['mobilenumber']) && !isset($this->request->get['id']))
		{
		    $this->load->model('matm/enrollments');
		    $enroll_info=$this->model_matm_enrollments->getEnrollmentByMobileNumber($this->request->post['mobilenumber']);
		    if(isset($enroll_info['customerid']) && !empty($enroll_info['customerid']) && ($enroll_info['status']==4 ||$enroll_info['status']==1||$enroll_info['status']==3))
		    {
		        $this->error['found_mobilenumber'] = $this->language->get('error_found_mobilenumber');
		    }
		}
		...
		if(!isset($this->request->post['dob']) || empty($this->request->post['dob']))
		{
		...
		if(!isset($this->request->post['district']) || empty($this->request->post['district']))
		{
		...
		if(!isset($this->request->post['area']) || empty($this->request->post['area']))
		{
		    $this->error['area'] = $this->language->get('error_area');
		}
		/*if(!isset($this->request->post['off_city']) || empty($this->request->post['off_city']))
		{
		    $this->error['off_city'] = $this->language->get('error_off_city');
		}
		if(!isset($this->request->post['off_state']) || empty($this->request->post['off_state']))
		{
		    $this->error['off_state'] = $this->language->get('error_off_state');
		}
		if(!isset($this->request->post['off_pincode']) || empty($this->request->post['off_pincode']))
		{
		    $this->error['off_pincode'] = $this->language->get('error_off_pincode');
		}
		if(!isset($this->request->post['off_district']) || empty($this->request->post['off_district']))
		{
		    $this->error['off_district'] = $this->language->get('error_off_district');
		}
		if(!isset($this->request->post['off_address']) || empty($this->request->post['off_address']))
		{
		    $this->error['off_address'] = $this->language->get('error_off_address');
		}
		if(!isset($this->request->post['off_area']) || empty($this->request->post['off_area']))
		{
		    $this->error['off_area'] = $this->language->get('error_off_area');
		}*/
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model'])) {
			...
			foreach ($results as $result) {
				...
				$product_options = $this->model_matm_enrollments->getProductOptions($result['product_id']);
