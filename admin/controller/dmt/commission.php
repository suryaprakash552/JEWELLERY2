...
class Commission extends \Opencart\System\Engine\Controller {
	...
	public function index() {
		$this->load->language('dmt/commission');

		$this->document->setTitle($this->language->get('heading_title'));

		$this->load->model('dmt/commission');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('dmt/commission', 'user_token=' . $this->session->data['user_token'] . $url, true));
		}
	...
	public function edit() {
       ...
       // print_r($this->request->post);
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->model_dmt_commission->editProduct($this->request->get['packageid'], $this->request->post);
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('dmt/commission', 'user_token=' . $this->session->data['user_token'] . $url, true)
		);

		$data['add'] = $this->url->link('dmt/commission.add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['delete'] = $this->url->link('dmt/commission.delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$product_total = $this->model_dmt_commission->getTotalProducts($filter_data,'DMT');
		$results = $this->model_dmt_commission->getProducts($filter_data,'DMT');
		//print_r($results);
        $i=1;
		foreach ($results as $result) 
		{
			...
			$data['products'][] = array(
				...
				'edit'       => $this->url->link('dmt/commission.edit', 'user_token=' . $this->session->data['user_token'] . '&packageid=' . $result['packageid'] . $url, true)
			);
			$i=$i+1;
		}
		...
		$data['sort_date'] = $this->url->link('dmt/commission', 'user_token=' . $this->session->data['user_token'] . '&sort=p.created' . $url, true);
		$data['sort_status'] = $this->url->link('dmt/commission', 'user_token=' . $this->session->data['user_token'] . '&sort=p.status' . $url, true);
		$data['sort_order'] = $this->url->link('dmt/commission', 'user_token=' . $this->session->data['user_token'] . '&sort=p.sort_order' . $url, true);
        ...
        $data['pagination'] = $this->load->controller('common/pagination', [
			...
			'page'  => $page,
			'limit' => $this->config->get('config_pagination_admin'),
			'url'   => $this->url->link('dmt/commission', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}')
		]);
		...
		$this->response->setOutput($this->load->view('dmt/dmt_dis_list', $data));
	}
	...
	protected function getForm() {
        ...
        if (isset($this->request->get['packageid'])&&($this->request->server['REQUEST_METHOD'] !='POST')) {
            $data['action'] = $this->url->link('dmt/commission.edit', 'user_token=' . $this->session->data['user_token'] . '&packageid=' . $this->request->get['packageid'] . $url, true);
        }
		
		$data['cancel'] = $this->url->link('dmt/commission', 'user_token=' . $this->session->data['user_token'] . $url, true);
        $product_info=array();
		if (isset($this->request->get['packageid']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$product_info = $this->model_dmt_commission->getProduct($this->request->get['packageid']);
		}
		//print_r($data);
		$data['user_token'] = $this->session->data['user_token'];
...
/*
        ...
        $data['services']=$this->model_dmt_commission->getAllServices();
		...
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');
		$this->response->setOutput($this->load->view('dmt/dmt_dis_form', $data));
	}

	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'dmt/commission')) {
