...
class Header extends \Opencart\System\Engine\Controller {
    ...
    public function index(): string {
        ...
        if (!isset($this->request->get['user_token']) || !isset($this->session->data['user_token']) || ($this->request->get['user_token'] != $this->session->data['user_token'])) {
            ...
            if ($user_info['logoimage'] && is_file(DIR_IMAGE . html_entity_decode($user_info['logoimage'], ENT_QUOTES, 'UTF-8'))) {
                $data['logoimage'] = $this->model_tool_image->resize($user_info['logoimage'], 45, 45);
            } else {
                $data['logoimage'] = $this->model_tool_image->resize('profile.png', 45, 45);
            }
            $data['setting_store'] = $this->url->link(
    'setting/store',
