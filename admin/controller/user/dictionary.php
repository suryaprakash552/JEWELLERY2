<?php
namespace Opencart\Admin\Controller\Info;
/**
 * Class User
 *
...
class User extends \Opencart\System\Engine\Controller {
	...
	public function index(): void {
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('info/user', 'user_token=' . $this->session->data['user_token'] . $url)
		];

		$data['add'] = $this->url->link('info/user.form', 'user_token=' . $this->session->data['user_token'] . $url);
		$data['delete'] = $this->url->link('info/user.delete', 'user_token=' . $this->session->data['user_token']);

		$data['list'] = $this->getList();

		// User Group
		$this->load->model('info/user_group');

		$data['user_groups'] = $this->model_info_user_group->getUserGroups();
		...
		$this->response->setOutput($this->load->view('info/user', $data));
	}
	...
	public function getList(): string {
		...
		$data['action'] = $this->url->link('info/user.list', 'user_token=' . $this->session->data['user_token'] . $url);
		...
		$this->load->model('info/user');

		$results = $this->model_info_user->getUsers($filter_data);

		foreach ($results as $result) {
			$data['users'][] = [
				'date_added' => date($this->language->get('date_format_short'), strtotime($result['date_added'])),
				'edit'       => $this->url->link('info/user.form', 'user_token=' . $this->session->data['user_token'] . '&user_id=' . $result['user_id'] . $url)
			] + $result;
		}
		...
		$data['sort_username'] = $this->url->link('info/user.list', 'user_token=' . $this->session->data['user_token'] . '&sort=username' . $url);
		$data['sort_name'] = $this->url->link('info/user.list', 'user_token=' . $this->session->data['user_token'] . '&sort=name' . $url);
		$data['sort_email'] = $this->url->link('info/user.list', 'user_token=' . $this->session->data['user_token'] . '&sort=u.email' . $url);
		$data['sort_user_group'] = $this->url->link('info/user.list', 'user_token=' . $this->session->data['user_token'] . '&sort=user_group' . $url);
		$data['sort_status'] = $this->url->link('info/user.list', 'user_token=' . $this->session->data['user_token'] . '&sort=u.status' . $url);
		$data['sort_date_added'] = $this->url->link('info/user.list', 'user_token=' . $this->session->data['user_token'] . '&sort=u.date_added' . $url);
		...
		$user_total = $this->model_info_user->getTotalUsers();

		$data['pagination'] = $this->load->controller('common/pagination', [
			'total' => $user_total,
			'page'  => $page,
			'limit' => $this->config->get('config_pagination_admin'),
			'url'   => $this->url->link('info/user.list', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}')
		]);
		...
		return $this->load->view('info/user_list', $data);
	}
	...
	public function form(): void {
		...
		$data['save'] = $this->url->link('info/user.save', 'user_token=' . $this->session->data['user_token']);
		$data['back'] = $this->url->link('info/user', 'user_token=' . $this->session->data['user_token'] . $url);

		if (isset($this->request->get['user_id'])) {
			$this->load->model('info/user');

			$user_info = $this->model_info_user->getUser($this->request->get['user_id']);
		}
		...
		$this->response->setOutput($this->load->view('info/user_form', $data));
	}
	...
	public function save(): void {
		...
		if (!$this->user->hasPermission('modify', 'info/user')) {
...
*/
		...
		$user_info = $this->model_info_user->getUserByEmail($post_info['email']);
		...
		if (!$json) {
			if (!$post_info['user_id']) {
				$json['user_id'] = $this->model_info_user->addUser($post_info);
			} else {
				$this->model_info_user->editUser($post_info['user_id'], $post_info);
			}
		...
		$user_info = $this->model_info_user->getLogininfoByUsername($post_info['username']);
		...
		if (!$json) {
			if (!$post_info['user_id']) {
				$json['user_id'] = $this->model_info_user->addLogininfo($post_info);
			} else {
				$this->model_info_user->editLogininfo($post_info['user_id'], $post_info);
			}
	...
	public function delete(): void {
		...
		if (!$json) {
			...
			foreach ($selected as $user_id) {
				$this->model_info_user->deleteUser($user_id);
			}
	...
	public function getAuthorize(): string {
		...
		if (isset($this->request->get['page']) && $this->request->get['route'] == 'info/user.login') {
			$page = (int)$this->request->get['page'];
		} else {
			$page = 1;
		}
		...
		$results = $this->model_info_user->getAuthorizes($user_id, ($page - 1) * $limit, $limit);
		...
		foreach ($results as $result) {
			$data['authorizes'][] = [
				...
				'delete'      => $this->url->link('info/user.deleteAuthorize', 'user_token=' . $this->session->data['user_token'] . '&user_authorize_id=' . $result['user_authorize_id'])
			] + $result;
		}

		$authorize_total = $this->model_info_user->getTotalAuthorizes($user_id);

		$data['pagination'] = $this->load->controller('common/pagination', [
			'total' => $authorize_total,
			'page'  => $page,
			'limit' => $limit,
			'url'   => $this->url->link('info/user.authorize', 'user_token=' . $this->session->data['user_token'] . '&user_id=' . $user_id . '&page={page}')
		]);
		...
		return $this->load->view('info/user_authorize', $data);
	}
	...
	public function deleteAuthorize(): void {
		...
		$authorize_info = $this->model_info_user->getAuthorize($user_authorize_id);
		...
		if (!$json) {
			$this->model_info_user->deleteAuthorizes($authorize_info['user_id'], $user_authorize_id);
	...
	public function getLogin(): string {
		...
		$results = $this->model_info_user->getLogins($user_id, ($page - 1) * $limit, $limit);

		foreach ($results as $result) {
			$data['logins'][] = ['date_added' => date($this->language->get('datetime_format'), strtotime($result['date_added']))] + $result;
		}

		$login_total = $this->model_info_user->getTotalLogins($user_id);

		$data['pagination'] = $this->load->controller('common/pagination', [
			'total' => $login_total,
			'page'  => $page,
			'limit' => $limit,
			'url'   => $this->url->link('info/user.login', 'user_token=' . $this->session->data['user_token'] . '&user_id=' . $user_id . '&page={page}')
		]);
		...
		return $this->load->view('info/user_login', $data);
	}
	...
	public function getLogininfo(): string {
		...
		$results = $this->model_info_user->getLogininfos($user_id, ($page - 1) * $limit, $limit);
		...
		foreach ($results as $result) {
			$data['logininfos'][] = [
				...
				'delete'      => $this->url->link('info/user.deleteLogininfo', 'user_token=' . $this->session->data['user_token'] . '&user_logininfo_id=' . $result['user_logininfo_id'])
			] + $result;
		}

		$authorize_total = $this->model_info_user->getTotalLogininfos($user_id);

		$data['pagination'] = $this->load->controller('common/pagination', [
			'total' => $authorize_total,
			'page'  => $page,
			'limit' => $limit,
			'url'   => $this->url->link('info/user.logininfo', 'user_token=' . $this->session->data['user_token'] . '&user_id=' . $user_id . '&page={page}')
		]);
		...
		return $this->load->view('info/user_logininfo', $data);
	}
