...
$data['festival_add'] = $this->url->link(
    'setting/setting.addFestival',
...
$year = date('Y');
$data['festivals'] = $this->model_setting_setting->getFestivalsByYear($year);
...
public function addFestival(): void {
    $this->load->language('setting/setting');
    $this->load->model('setting/setting');
    ...
    if ($this->request->server['REQUEST_METHOD'] === 'POST') {
        if (!empty($input['date']) && !empty($input['festival'])) {
            try {
                $this->model_setting_setting->addFestival([
                    'date'     => $input['date'],
                ...
                // Do NOT echo/print the exception — return it in JSON and optionally log it
                $json['success'] = false;
                $json['error'] = 'DB error: ' . $e->getMessage();
                // Optional: $this->log->write('Festival insert error: ' . $e->getMessage());
            }
        } else {
            $json['success'] = false;
            $json['error'] = 'Missing date or festival';
        }
    } else {
        $json['success'] = false;
