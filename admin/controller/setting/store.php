...
class Store extends \Opencart\System\Engine\Controller {
    ...
    true
		...
		if (isset($this->request->get['store_id'])) {
			...
			$year = date('Y');
			$data['festivals'] = $this->model_setting_store->getFestivalsByYear($year);
    ...
    if ($this->request->server['REQUEST_METHOD'] == 'POST') {
        ...
        if (!empty($data['date']) && !empty($data['festival'])) {
            // Load model
            $this->load->model('setting/store');

            // Save festival
            $this->model_setting_store->addFestival($data);
