...
class Category extends \Opencart\System\Engine\Model {
	...
	public function addCategory(array $data): int {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "category` SET `image` = '" . $this->db->escape((string)$data['image']) . "', `parent_id` = '" . (int)$data['parent_id'] . "', `sort_order` = '" . (int)$data['sort_order'] . "',`offer` = '" . (int)$data['offer'] . "', `offer_from` = " . (!empty($data['offer_from']) ? "'" . $this->db->escape($data['offer_from']) . "'" : "NULL") . ", `offer_to` = " . (!empty($data['offer_to']) ? "'" . $this->db->escape($data['offer_to']) . "'" : "NULL") . ",`offer_percentage` = '" . (float)$data['offer_percentage'] . "', `gst` = '" . (float)$data['gst'] . "',`status` = '" . (bool)($data['status'] ?? 0) . "'");
	...
	public function editCategory(int $category_id, array $data): void {
		$this->db->query("UPDATE `" . DB_PREFIX . "category` SET `image` = '" . $this->db->escape((string)$data['image']) . "', `parent_id` = '" . (int)$data['parent_id'] . "', `offer` = '" . (int)$data['offer'] . "',`offer_from` = " . (!empty($data['offer_from']) ? "'" . $this->db->escape($data['offer_from']) . "'" : "NULL") . ",`offer_to` = " . (!empty($data['offer_to']) ? "'" . $this->db->escape($data['offer_to']) . "'" : "NULL") . ", `offer_percentage` = '" . (float)$data['offer_percentage'] . "',`gst` = '" . (float)$data['gst'] . "',`sort_order` = '" . (int)$data['sort_order'] . "', `status` = '" . (bool)($data['status'] ?? 0) . "' WHERE `category_id` = '" . (int)$category_id . "'");
