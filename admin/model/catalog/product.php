...
class Product extends \Opencart\System\Engine\Model {
    ...
    // get highest short_code used (only numbers)
    $query = $this->db->query("
        SELECT MAX(short_code) AS max_code
        FROM `" . DB_PREFIX . "product`
        WHERE short_code REGEXP '^[0-9]{1,2}$'
    ");

    if ($query->row['max_code']) {
        return (int)$query->row['max_code'] + 1;
    }

    return 10; // starting number (10, 11, 12, ...)
}

	public function addProduct(array $data): int {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "product` SET `master_id` = '" . (int)$data['master_id'] . "',`model` = '" . $this->db->escape((string)$data['model']) . "',`rack_code` = '" . $this->db->escape((string)$data['rack_code']) . "',`upc` = '" . $this->db->escape((string)$data['upc']) . "',`sku` = '" . $this->db->escape((string)$data['sku']) . "', `box_id` = '" . $this->db->escape((string)$data['box_id']) . "',`location` = '" . $this->db->escape((string)$data['location']) . "', `variant` = '" . $this->db->escape(!empty($data['variant']) ? json_encode($data['variant']) : '') . "', `override` = '" . $this->db->escape(!empty($data['override']) ? json_encode($data['override']) : '') . "', `quantity` = '" . (int)$data['quantity'] . "', `minimum` = '" . (int)$data['minimum'] . "', `subtract` = '" . (isset($data['subtract']) ? (bool)$data['subtract'] : 0) . "', `stock_status_id` = '" . (int)$data['stock_status_id'] . "', `date_available` = '" . $this->db->escape((string)$data['date_available']) . "', `manufacturer_id` = '" . (int)$data['manufacturer_id'] . "', `shipping` = '" . (isset($data['shipping']) ? (bool)$data['shipping'] : 0) . "',`received_price` = '" . (float)$data['received_price'] . "',`r_tax` = '" . (float)$data['r_tax'] . "', `price` = '" . (float)$data['price'] . "',`additional_price` = '" . (float)$data['additional_price'] ."',`special_price` = '" . (float)$data['special_price'] . "', `wholesale_price` = '" . (float)$data['wholesale_price'] . "',`max_quantity` = '" . (int)$data['max_quantity'] . "',`r_tag` = '" . $this->db->escape($data['r_tag']) . "',`w_tag` = '" . $this->db->escape($data['w_tag']) . "',`points` = '" . (int)$data['points'] . "', `weight` = '" . (float)$data['weight'] . "', `weight_class_id` = '" . (int)$data['weight_class_id'] . "', `length` = '" . (float)$data['length'] . "', `width` = '" . (float)$data['width'] . "', `height` = '" . (float)$data['height'] . "', `length_class_id` = '" . (int)$data['length_class_id'] . "', `status` = '" . (bool)($data['status'] ?? 0) . "', `tax_class_id` = '" . (int)$data['tax_class_id'] . "', `sort_order` = '" . (int)$data['sort_order'] . "', `date_added` = NOW(), `date_modified` = NOW()");
...
public function getBoxById(int $product_id): ?array {
	...
	public function editProduct(int $product_id, array $data): void {
		$this->db->query("UPDATE `" . DB_PREFIX . "product` SET `model` = '" . $this->db->escape((string)$data['model']) . "',`rack_code` = '" . $this->db->escape((string)$data['rack_code']) . "',`upc` = '" . $this->db->escape((string)$data['upc']) . "', `sku` = '" . $this->db->escape((string)$data['sku']) . "',`box_id` = '" . $this->db->escape((string)$data['box_id']) . "', `short_code` = '" . $this->db->escape((string)$data['short_code']) . "',`location` = '" . $this->db->escape((string)$data['location']) . "', `variant` = '" . $this->db->escape(!empty($data['variant']) ? json_encode($data['variant']) : '') . "', `override` = '" . $this->db->escape(!empty($data['override']) ? json_encode($data['override']) : '') . "', `quantity` = '" . (int)$data['quantity'] . "', `minimum` = '" . (int)$data['minimum'] . "', `subtract` = '" . (isset($data['subtract']) ? (bool)$data['subtract'] : 0) . "', `stock_status_id` = '" . (int)$data['stock_status_id'] . "', `image` = '" . $this->db->escape((string)$data['image']) . "', `date_available` = '" . $this->db->escape((string)$data['date_available']) . "', `manufacturer_id` = '" . (int)$data['manufacturer_id'] . "', `shipping` = '" . (isset($data['shipping']) ? (bool)$data['shipping'] : 0) . "',`received_price` = '" . (float)$data['received_price'] . "',`r_tax` = '" . (float)$data['r_tax'] . "', `price` = '" . (float)$data['price'] . "',`additional_price` = '" . (float)$data['additional_price'] . "',`special_price` = '" . (float)$data['special_price'] . "', `wholesale_price` = '" . (float)$data['wholesale_price'] . "',`max_quantity` = '" . (int)$data['max_quantity'] . "',`r_tag` = '" . $this->db->escape($data['r_tag']) . "',`w_tag` = '" . $this->db->escape($data['w_tag']) . "',`points` = '" . (int)$data['points'] . "', `weight` = '" . (float)$data['weight'] . "', `weight_class_id` = '" . (int)$data['weight_class_id'] . "', `length` = '" . (float)$data['length'] . "', `width` = '" . (float)$data['width'] . "', `height` = '" . (float)$data['height'] . "', `length_class_id` = '" . (int)$data['length_class_id'] . "', `status` = '" . (bool)($data['status'] ?? 0) . "', `tax_class_id` = '" . (int)$data['tax_class_id'] . "', `sort_order` = '" . (int)$data['sort_order'] . "', `date_modified` = NOW() WHERE `product_id` = '" . (int)$product_id . "'");
    ...
    if ($query->num_rows) {
        ...
        // Safely decode JSON with fallback to empty array
...
public function getProductOptions($product_id = []) {
	...
	public function getProducts(array $data = []): array {
		$sql = "SELECT * FROM `" . DB_PREFIX . "product` p LEFT JOIN `" . DB_PREFIX . "product_description` pd ON (p.product_id = pd.product_id)LEFT JOIN `" . DB_PREFIX . "pts_pos_product` ppq ON (p.product_id = ppq.product_id)";
		...
		if (!empty($data['filter_rack_code'])) {
            $sql .= " AND LCASE(p.rack_code) LIKE '%" .
                $this->db->escape(oc_strtolower($data['filter_rack_code'])) . "%'";
        }
        
        if (!empty($data['filter_box_id'])) {
            $box_id = $this->db->escape($data['filter_box_id']);
        
            $sql .= " AND (p.box_id LIKE '%" . $box_id . "%' 
                      OR p.product_id = '" . (int)$box_id . "')";
        }

        
        if (!empty($data['filter_barcode'])) {
            $barcode = $this->db->escape(oc_strtolower($data['filter_barcode']));
            $sql .= " AND (
                LCASE(p.sku) LIKE '%{$barcode}%'
                OR LCASE(p.upc) LIKE '%{$barcode}%'
            )";
        }
...
elseif (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
	...
	public function getTotalProducts(array $data = []): int {
		...
		// Rack Code filter
            ...
            // Box ID filter
            if (!empty($data['filter_box_id'])) {
                $sql .= " AND LCASE(p.box_id) LIKE '%" .
                    $this->db->escape(oc_strtolower($data['filter_box_id'])) . "%'";
            }
            
            // Barcode filter (SKU / UPC)
            if (!empty($data['filter_barcode'])) {
                $barcode = $this->db->escape(oc_strtolower($data['filter_barcode']));
                $sql .= " AND (
                    LCASE(p.sku) LIKE '%{$barcode}%'
...
public function getNextUnitBarcode(): int {
    $query = $this->db->query("
        SELECT MAX(sku) AS max_sku
        FROM " . DB_PREFIX . "product
        WHERE sku >= 10000001
    ");

    if (!empty($query->row['max_sku'])) {
        return (int)$query->row['max_sku'] + 1;
    }
...
public function generateBoxBarcode(): int {
    ...
    $seconds = floor($micro);
    $milliseconds = (int)(($micro - $seconds) * 1000);

    // Use 2-digit year to make it 12 digits
    $dateTime = date('ymdHis', $seconds); // 12 digits

    // Final 15-digit barcode
    $barcode = $dateTime . str_pad($milliseconds, 3, '0', STR_PAD_LEFT);
