...
public function getSellerSummary(array $data = []): array {
    ...
    FROM (
        SELECT
            ...
            /* Purchase cost (received price + tax) */
            SUM(op.quantity * p.received_price)         AS r_price,
            SUM(op.quantity * p.r_tax)                  AS r_tax,
            SUM(op.quantity * p.received_price + op.quantity * p.r_tax) AS r_total,

            /* s_price: sub_total for both completed and return */
            MAX(inv.sub_total)                          AS s_price,

            /* s_tax: 0 for return completed (6), normal for completed (5) */
            CASE
                WHEN o.order_status_id = 6 THEN 0
                ELSE MAX(inv.total_tax)
            END                                         AS s_tax,
        ...
        /* Only completed (5) and return completed (6) orders */
...
FROM (
    SELECT 
        ...
        /* s_price: for return completed = s_price, else normal */
        CASE 
            WHEN o.order_status_id = 6 THEN MAX(inv.sub_total)
            ELSE MAX(inv.sub_total)
        END AS s_price,

        /* s_tax: for return completed = 0, else normal */
        CASE 
            WHEN o.order_status_id = 6 THEN 0
            ELSE MAX(inv.total_tax)
        END AS s_tax,

        /* s_total: for return completed = s_price (sub_total), else normal */
        CASE 
            WHEN o.order_status_id = 6 THEN MAX(inv.sub_total)
            ELSE MAX(inv.total_received)
        END AS s_total,

        /* discount: for return completed = sum of discount, else normal */
        CASE 
            WHEN o.order_status_id = 6 THEN MAX(inv.discount)
            ELSE MAX(inv.discount)
        END AS discount,

        /* cash: for return completed = s_total (sub_total), else normal */
        CASE 
            WHEN o.order_status_id = 6 THEN MAX(inv.sub_total)
            ELSE GREATEST(
                MAX(inv.cash_amount) -
        ...
        /* upi: for return completed = 0, else normal */
        CASE 
            WHEN o.order_status_id = 6 THEN 0
            ELSE GREATEST(
                MAX(inv.upi_amount) -
        ...
        /* advance: for return completed = 0, else normal */
        CASE 
            WHEN o.order_status_id = 6 THEN 0
            ELSE MAX(inv.advance_used)
        END AS advance,

        /* due: for return completed = 0, else normal */
        CASE 
            WHEN o.order_status_id = 6 THEN 0
