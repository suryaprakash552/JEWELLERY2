...
public function addFestival(array $data): void {
    $this->db->query("INSERT INTO `" . DB_PREFIX . "festival`
        SET `date` = '" . $this->db->escape($data['date']) . "',
            `festival` = '" . $this->db->escape($data['festival']) . "'");
}

public function getFestivalsByYear($year): array {
    $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "festival`
