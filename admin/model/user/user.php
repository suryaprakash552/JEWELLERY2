...
class User extends \Opencart\System\Engine\Model {
	...
	public function addUser(array $data): int {
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX . "user` SET `username` = '" . $this->db->escape((string)$data['username']) . "', `user_group_id` = '" . (int)$data['user_group_id'] . "', `password` = '" . $this->db->escape(password_hash(html_entity_decode($data['password'], ENT_QUOTES, 'UTF-8'), PASSWORD_DEFAULT)) . "', `firstname` = '" . $this->db->escape((string)$data['firstname']) . "', `lastname` = '" . $this->db->escape((string)$data['lastname']) . "', `email` = '" . $this->db->escape((string)$data['email']) . "', `logoimage` = '" . $this->db->escape((string)$data['logoimage']) . "', `status` = '" . (bool)($data['status'] ?? 0) . "', `date_added` = NOW()");

		$user_id = $this->db->getLastId();

    //emp_info table
      $this->db->query("INSERT INTO `" . DB_PREFIX . "emp_info` SET
                                                       `user_id` = '" . (int)$user_id . "',
                                                       `employeeid` = '" . $this->db->escape((string)$data['employeeid']) . "',
                                                       `mobilenumber` = '" . $this->db->escape((string)($data['mobilenumber'])) . "',
                                                       `gender` = '" . (int)$data['user_gender'] . "',
                                                       `presentadd` = '" . $this->db->escape((string)($data['presentadd'])) . "',
                                                       `permanentadd` = '" . $this->db->escape((string)($data['permanentadd'])) . "',
                                                       `zone_id` = '" . (int)($data['zone_id']) . "',
                                                       `designation_id` = '" . (int)($data['designation_id']) . "',
                                                       `employmenttype_id` = '" . (int)($data['employmenttype_id']) . "',
                                                       `employmentstatus_id` = '" . (int)($data['employmentstatus_id']) . "',
                                                       `sourceofhire_id` = '" . (int)($data['sourceofhire_id']) . "',
                                                       `experience` = '" . (int)($data['experience']) . "',
                                                       `pan` = '" . $this->db->escape((string)($data['pan'])) . "',
                                                       `aadhar` = '" . $this->db->escape((string)($data['aadhar'])) . "',
                                                       `panimage` = '" . $this->db->escape((string)($data['panimage'])) . "',
                                                       `aadharimage` = '" . $this->db->escape((string)($data['aadharimage'])) . "',
                                                       `date_of_birth` = '" . $this->db->escape((string)($data['date_of_birth'])) . "',
                                                       `date_of_joining` = '" . $this->db->escape((string)($data['date_of_joining'])) . "',
                                                       `reportingempid` = '" . (int)($data['reportingempid']) . "',
                                                       `referredby_id` = '" . (int)($data['referredby_id']) . "'");


    // user_salary table
        $this->db->query("INSERT INTO `" . DB_PREFIX . "user_salary` SET 
                                                        `user_id` = '" . (int)$user_id . "',
                                                        `annualctc` = '" . (float)$data['annualctc'] . "',
                                                       `basic` = '" . (int)$data['basic'] . "',
                                                        `hra` = '" . (int)$data['hra'] . "',
                                                        `variablepay` = '" . (int)$data['variablepay'] . "',
                                                        `conveyance` = '" . (float)$data['conveyance'] . "',
                                                        `pf` = '" . (int)$data['pf'] . "',
                                                        `calbasic` = '" . (float)$data['calbasic'] . "',
                                                        `calhra` = '" . (float)$data['calhra'] . "',
                                                        `calvariablepay` = '" . (float)$data['calvariablepay'] . "',
                                                        `calpf` = '" . (float)$data['calpf'] . "',
                                                        `conveyance` = '" . (float)$data['conveyance'] . "',
                                                        `fixed` = '" . (float)$data['fixed'] . "',
                                                        `total` = '" . (float)$data['total'] . "',
                                                        `ybasic` = '" . (float)$data['ybasic'] . "',
                                                        `yhra` = '" . (float)$data['yhra'] . "',
                                                        `yvariablepay` = '" . (float)$data['yvariablepay'] . "',
                                                        `ypf` = '" . (float)$data['ypf'] . "',
                                                        `yconveyance` = '" . (float)$data['yconveyance'] . "',
                                                        `yfixed` = '" . (float)$data['yfixed'] . "',
                                                        `ytotal` = '" . (float)$data['ytotal'] . "'");
    ...
    if (!empty($data['password'])) {
     ...
     $this->db->query("DELETE FROM `" . DB_PREFIX . "emp_info` WHERE user_id = '" . (int)$user_id . "'");
    ...
    // USER SALARY 
    $this->db->query("DELETE FROM `" . DB_PREFIX . "user_salary` WHERE user_id = '" . (int)$user_id . "'");
	...
	public function getEmployees(int $user_id) {
    $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "emp_info WHERE user_id = '" . (int)$user_id . "'");
    
    return $query->row;
   }
   public function getSalaries(int $user_id) {
    $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "user_salary WHERE user_id = '" . (int)$user_id . "'");
	...
	public function getZones(): array {
		...
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "zone_description` ");

		return $query->rows;
	}
	
	public function getDesignations(): array {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "designation` ");

		return $query->rows;
	}
	
	public function getEmploymenttypes(): array {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "employmenttype` ");

		return $query->rows;
	}
	
	public function getEmploymentsstatus(): array {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "employmentsstatus` ");

		return $query->rows;
	}
	
	public function getSourceofhire(): array {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "sourceofhire` ");
