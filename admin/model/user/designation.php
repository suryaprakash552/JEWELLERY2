 ...
 */
class Designation extends \Opencart\System\Engine\Model {
	/**
	 * Add Language
	 *
	 ...
	 */
	public function addDesignation(array $data): int {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "designation` SET `name` = '" . $this->db->escape((string)$data['name']) . "'");

		$this->cache->delete('designation');
	 ...
	 */
	public function editDesignation(int $designation_id, array $data): void {
		$this->db->query("UPDATE `" . DB_PREFIX . "designation` SET `name` = '" . $this->db->escape((string)$data['name']) . "' WHERE `designation_id` = '" . (int)$designation_id . "'");
	 ...
	 */
	public function deleteDesignation(int $designation_id): void {
	...
	/*public function getEmploymenttypeByCode(string $code): array {
		...
		public function getDesignationByName(string $name): array {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "designation` WHERE `name` = '" . $this->db->escape($name) . "'");
	 ...
	 */
	public function getDesignations(array $data = []): array {
		...
		if (!$results) {
			...
			$this->cache->set('designation.' . md5($sql), $results);
		}
	 ...
	 */
	public function getDesignationsByExtension(string $extension): array {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "designation` WHERE `extension` = '" . $this->db->escape($extension) . "'");
	 ...
	 */
	public function getTotalDesignations(): int {
		$query = $this->db->query("SELECT COUNT(*) AS `total` FROM `" . DB_PREFIX . "designation`");
