...
class Paymentrequest extends \Opencart\System\Engine\Model {
    ...
    public function getOrderHistories($order_id, $start = 0, $limit = 10) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_payment_history oh WHERE oh.order_id = '" . (int)$order_id . "' ORDER BY oh.date_added DESC LIMIT " . (int)$start . "," . (int)$limit);

		return $query->rows;
	}
	
	public function getTotalOrderHistories($order_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "order_payment_history WHERE order_id = '" . (int)$order_id . "'");

		return $query->row['total'];
	}
	
	public function getProduct($order_id) {
		$query = $this->db->query("SELECT DISTINCT p.*,c.firstname,c.email,c.telephone FROM " . DB_PREFIX . "manage_payment_requests p INNER JOIN " . DB_PREFIX . "customer c ON (c.customer_id=p.customerid) WHERE p.id = '" . (int)$order_id . "' AND p.language_id = '" . (int)$this->config->get('config_language_id') . "'");

		return $query->row;
	}
    
	public function getProducts($data = array()) {
		$sql = "SELECT * FROM " . DB_PREFIX . "manage_payment_requests p WHERE p.language_id = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function getTotalProducts($data = array()) {
		...
		$sql = "SELECT COUNT(DISTINCT p.id) total FROM " . DB_PREFIX . "manage_payment_requests p WHERE p.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		...
		$sql = "SELECT sum(p.amount) total FROM " . DB_PREFIX . "manage_payment_requests p WHERE p.language_id = '" . (int)$this->config->get('config_language_id') . "'";
