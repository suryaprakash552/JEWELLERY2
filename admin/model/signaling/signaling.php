<?php
namespace Opencart\Admin\Model\Signaling;
/**
 * Class Customer
 *
 ...
 */
class Signaling extends \Opencart\System\Engine\Model {
	/**
	 * Add Customer
	 *
	 ...
	 */
	public function addSignaling(array $data): int {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "signaling` SET 
		    `theme` = '" . (string)$this->db->escape($data['theme']) . "', 
		    `amc` = '" . (string)$data['amc'] . "', 
		    `fund` = '" . (string)$data['fund'] . "',
		    `exit_load` = '" . (string)$data['exit_load'] . "',
		    `current_signal_id` = '" . (int)$data['current_signal_id'] . "',
		    `created_date` = '" . (string)$data['active_date'] . "',
		    `t_signaling` = '" . (string)$data['tsignal'] . "'");
	 ...
	 */
	public function editSignaling(int $signaling_id, array $data): void {
		$this->db->query("UPDATE `" . DB_PREFIX . "signaling` SET 
		...
		`created_date` = '" . (string)$data['active_date'] . "', 
		`t_signaling` = '" . (string)$data['tsignal'] . "'
		WHERE `signaling_id` = '" . (int)$signaling_id ."'"); 
...
// 		}
	 ...
	 */
	public function deleteSignaling(int $signaling_id): void {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "signaling` WHERE `signaling_id` = '" . (int)$signaling_id . "'");
	}
	 ...
	 */
	public function getSignaling(int $signaling_id): array {
		$query = $this->db->query("SELECT DISTINCT * FROM `" . DB_PREFIX . "signaling` WHERE `signaling_id` = '" . (int)$signaling_id . "'");
	 ...
	 * $customer_info = $this->model_customer_customer->getCustomer($customer_id);
	 */
	public function getCurrentSignalsById(int $current_signal_id): array {
		$query = $this->db->query("SELECT DISTINCT * FROM `" . DB_PREFIX . "current_signal` WHERE `current_signal_id` = '" . (int)$current_signal_id . "'");
	 ...
	 */
	public function getSignalings(array $data = []): array {
		$sql = "SELECT s.*,cs.current_signal FROM `" . DB_PREFIX . "signaling` `s` LEFT JOIN `" . DB_PREFIX . "current_signal` `cs` ON (`s`.`current_signal_id` = `cs`.`current_signal_id`) WHERE `cs`.`language_id` = '" . (int)$this->config->get('config_language_id') . "'";
        //echo $sql;
		if (!empty($data['filter_theme'])) {
			$sql .= " AND LCASE(`s`.`theme`) LIKE '" . $this->db->escape('%' . oc_strtolower($data['filter_theme']) . '%') . "'";
		}

		if (!empty($data['filter_amc'])) {
			$sql .= " AND LCASE(`s`.`amc`) LIKE '" . $this->db->escape(oc_strtolower($data['filter_amc']) . '%') . "'";
		}

		if (!empty($data['filter_current_signal_id'])) {
			$sql .= " AND `s`.`current_signal_id` = '" . (int)$data['filter_current_signal_id'] . "'";
		}
    	
    	if (!empty($data['filter_update_date_from'])) {
			$sql .= " AND DATE(`s`.`created_date`) >= DATE('" . $this->db->escape((string)$data['filter_update_date_from']) . "')";
		}

		if (!empty($data['filter_update_date_to'])) {
			$sql .= " AND DATE(`s`.`created_date`) <= DATE('" . $this->db->escape((string)$data['filter_update_date_to']) . "')";
		}
		$sort_data = [
			's.theme.',
			's.amc',
	 ...
	 * $results = $this->model_customer_customer->getCustomers($filter_data);
	 */
	public function getCurrent_signals(): array {
		$sql = "SELECT * FROM `" . DB_PREFIX . "current_signal` `cs` ";
	 ...
	 */
	public function getTotalSignaling(array $data = []): int {
		$sql = "SELECT COUNT(*) AS `total` FROM `" . DB_PREFIX . "signaling` `s`";

		$implode = [];

		if (!empty($data['filter_theme'])) {
			$implode[] = "LCASE(CONCAT(`s`.`theme`)) LIKE '" . $this->db->escape('%' . oc_strtolower($data['filter_theme']) . '%') . "'";
		}

		if (!empty($data['filter_amc'])) {
			$implode[] = "LCASE(`s`.`amc`) LIKE '" . $this->db->escape(oc_strtolower($data['filter_amc']) . '%') . "'";
		}

		if (!empty($data['filter_current_signal_id'])) {
			$implode[] = "`s`.`current_signal_id` = '" . (int)$data['filter_current_signal_id'] . "'";
		}
	...
	/*	if (!empty($data['filter_ip'])) {
		...
		if (!empty($data['filter_update_date_from'])) {
			$implode[] = "DATE(`s`.`created_date`) >= DATE('" . $this->db->escape((string)$data['filter_update_date_from']) . "')";
		}

		if (!empty($data['filter_update_date_to'])) {
			$implode[] = "DATE(`s`.`created_date`) <= DATE('" . $this->db->escape((string)$data['filter_update_date_to']) . "')";
		}
